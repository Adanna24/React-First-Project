  ...
  const getmemeimg = () => {
    ...
    const savememe = () => {
      if(memeinput.topText.trim().length || memeinput.bottomText.trim().length > 0) {
        ...
        setmemeinput( {topText:"", bottomText:"", url:"https://i.pinimg.com/originals/2d/7d/0f/2d7d0fee592a9ea7341ccae86627e379.jpg"})
      }
      else {
    ...
    const Downloadmeme = () => {
      window.alert("Cant Download, Function not working for Now")
    }
              ...
              {memes.map(meme => (
                <div className='meme-whole'>
                  ...
                  <div className='btns'>
                    ...
                    <button className='btn-sm green w-100'  onClick={Downloadmeme}>Download</button>
                  </div>
                </div>
              ) ) }
            </div>
